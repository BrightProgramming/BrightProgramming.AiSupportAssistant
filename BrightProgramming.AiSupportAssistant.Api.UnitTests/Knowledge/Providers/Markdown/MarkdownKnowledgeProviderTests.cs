...
public class MarkdownKnowledgeProviderTests
{
    ...
    public async Task GetKnowledgeAsync_LoadsMarkdownFilesRecursivelyAndExcludesOtherFiles()
    {
        using var directory = new TemporaryDirectory();
        File.WriteAllText(
            Path.Combine(directory.Path, "top-level.md"),
            "top-level content");
        ...
        File.WriteAllText(
            Path.Combine(nestedDirectory.FullName, "nested.md"),
            "nested content");

        File.WriteAllText(
            Path.Combine(nestedDirectory.FullName, "ignored.txt"),
            "ignore me");
        ...
        Assert.Contains(
            result.Documents,
            x => x.Name == "top-level.md" &&
                 x.Content == "top-level content");

        Assert.Contains(
            result.Documents,
            x => x.Name == "nested.md" &&
                 x.Content == "nested content");
    }
    ...
    public async Task GetKnowledgeAsync_CachesDocumentsAfterFirstLoad()
    {
        ...
        File.WriteAllText(
            Path.Combine(directory.Path, "first.md"),
            "first");
        ...
        File.WriteAllText(
            Path.Combine(directory.Path, "second.md"),
            "second");
        ...
        Assert.Equal(
            "first.md",
            Assert.Single(second.Documents).Name);
    }

    private static MarkdownKnowledgeProvider CreateProvider(string path)
    {
        var repository = new MarkdownKnowledgeRepository(
            Options.Create(new KnowledgeSourceOptions
            {
                Path = path
            }),
            NullLogger<MarkdownKnowledgeRepository>.Instance);

        return new MarkdownKnowledgeProvider(repository);
    }
